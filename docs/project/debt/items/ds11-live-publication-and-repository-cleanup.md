...
coordinate in transient thread output. A credential-free follow-up resolves that
rendering gap: private setup observations and the exact atomic push now sanitize
ordinary exceptions, retain only closed evidence, and durably spend before the
runner can execute. All local plaintext and authority were erased after a fresh
encrypted transfer to a new durable Puck custodian. This target is a second open
exact-target cleanup obligation; custody is not cleanup authority.
