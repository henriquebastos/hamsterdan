# CV21 delivery sequence

CV21 proceeds through twelve progressively deepened outer-system tracer bullets.
The current Net is real at every workflow step; no tracer replaces it with a
mock or begins a new workflow implementation.

## Tracer acceptance rule

A CV21 Delivery Story closes only when it:
...
6. adds direct correspondence for every newly introduced external or bridge
   seam; and
7. leaves the current runtime and state untouched.
...
The exact work declared by retained workflow History causes the exact delivered
coding result to enter Git publication. Object creation, commit/ref identity,
lookup, CAS, orphan handling, and the exact original terminal are causally
checked across reconstruction.
...
Timer commands, acknowledgements, due claims, History acceptance, and delivered
marks are separate durable cuts. Logical time and the production clock use the
same explicit capability without hidden sleeps or drains.
...
Repeated bounded passes discover eligible unknown open PRs only in configured
repositories. List rows are hints; an exact read precedes idempotent
registration, enqueue, and common admission.
...
The complete outer system passes accepted deterministic journeys, generated
schedule/recovery campaigns, bridge-family correspondence, checker mutation,
resource bounds, available real seams, and distribution checks. It remains
non-selectable and performs no cutover.

## API-strengthening rule
...
- strict commands/observations, checker derivation, resource bounds, and
  correspondence; and
- its bridge-family census row and exact old-type containment.

The Plan updates CV21's canonical owner before code. It cannot recover a missing
choice from CV20 or the pending-rulings register.

## Stop conditions

Stop the selected tracer and return to a Navigator checkpoint if:

- mounting the current Net requires importing a current outer application
  module;
- a useful new boundary cannot be expressed without exposing a current type;
- translation would need to recreate a workflow decision rather than convert a
  representation;
- the required public Petrus bounded recovery seam does not exist;
- the tracer needs a second major policy or causal chain to become coherent;
- a current behavior contradicts an accepted CV21 outer contract; or
- work would make CV21 selectable, touch current state, or perform a shared
  external action without its explicit approval.
