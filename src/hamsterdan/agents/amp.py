...
class AmpExecuteRunner:
    ...
    def _instructions(kind: str, request: object) -> str:
        ...
        semantics = {
            ...
            "conversation": "Choose only types and exact argument names declared in request.allowed_intents. Put the concrete edit and its scope in a mutation's request argument; omit conversational directions about staging, confirmation, or whether to execute because the host enforces those controls. Copy declared mutation metadata. For an explicit mutation request, emit the mutation intent with confirmation=false so the host can stage it and request exact confirmation. Set confirmation=true only when the declaration says confirmation_available and the comment explicitly confirms the pending mutation. Never treat an unconfirmed request as authorized execution.",
