...
from hamsterdan2.host.delivery import MAX_RETAINED_DELIVERIES, DeliveryCompletionCustody, DeliveryCustody
...
def fold_readiness_observation(
    registered: RegisteredPullRequest,
    *,
    provider_route_id: ProviderRouteId,
    delivery_id: DeliveryId,
    ...
    return runtime.fold_accepted_observation(
        provider_route_id=provider_route_id,
        delivery_id=delivery_id,
        subject=registered.subject,
    )
...
def verify_readiness_observation(
    ...
    *,
    ...
    return runtime.verify_folded_observation(
        provider_route_id=provider_route_id,
        delivery_id=delivery_id,
        subject=registered.subject,
    )
...
def build_observation_acceptance_authority(
    *,
    ...
    return ObservationAcceptanceAuthority(
        ...
        completion_custody=DeliveryCompletionCustody.from_path(
        ...
        fold_readiness=partial(
            fold_readiness_observation,
            state_root=state_root,
            dispatch_path=state_root / "dispatch.sqlite3",
            ingress_path=ingress_path,
            maximum_manifests=maximum_manifests,
        ),
        verify_readiness=partial(
