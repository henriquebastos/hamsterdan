...
class ScenarioProvider:
    def __init__(self, *, first_attempt_flake: bool = False) -> None:
        ...
        self.first_attempt_flake = first_attempt_flake
        self.run_attempt = 1
        self.run_conclusion = "failure" if first_attempt_flake else "success"
    ...
    def complete_rerun(self) -> None:
        if not self.first_attempt_flake or self.rerun_requests != 1:
        ...
        if (self.run_attempt, self.run_conclusion) != (1, "failure"):
            raise AssertionError("only failed attempt 1 can advance to the flaky retry")
        self.run_attempt = 2
...
def _run_journey(
    ...
    *,
    first_attempt_flake: bool,
) -> JourneyResult:
    provider = ScenarioProvider(first_attempt_flake=first_attempt_flake)
    runner = ScenarioRunner()
    ...
    if first_attempt_flake:
        provider.complete_rerun()
        follow_up, follow_up_body = str(uuid.uuid4()), workflow_envelope()
...
def run_clean_green(
    ...
    return _run_journey(root, monkeypatch, topology, first_attempt_flake=False)
...
def run_first_attempt_flake(
    ...
    return _run_journey(root, monkeypatch, topology, first_attempt_flake=True)
